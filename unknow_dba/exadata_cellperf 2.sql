...
select *
  from (select c.cell_path,
         ...
         where sw.p1text = 'cellhash#'
           and c.cell_hashval = sw.p1
         order by 8 desc)
 where rownum < 21;
...
select c.cell_path,
       sum(a.time_waited) TOTAL_WAIT_TIME,
       avg(a.time_waited) AVG_WAIT_TIME
  from v$cell c, gv$active_session_history a
 where a.p1text = 'cellhash#'
   and c.cell_hashval = a.p1
...
select *
  from (select to_char(a.sample_time, 'Mondd_hh24mi') minute,
               c.cell_path,
               sum(a.time_waited) TOTAL_WAIT_TIME,
               avg(a.time_waited) AVG_WAIT_TIME
          from v$cell c, gv$active_session_history a
...
select * from (select * from (select distinct dhs.snap_id, 
								...
								from dba_hist_snapshot dhs, 
									 dba_hist_cell_disk_summary cds, 
									 v$cell_disk cd
							   where (cds.cell_hash = cd.cell_hash and cds.disk_id = cd.disk_id)
								 and dhs.snap_id = cds.snap_id and to_char(dhs.begin_interval_time,'mondd_hh24') in (select hour from (select to_char(a.sample_time,'mondd_hh24') hour, 
																																			  avg(a.time_waited) avg_wait_time
																																		 from dba_hist_active_sess_history a
																																		where event like 'cell%' or event like 'db file%' 
...
select *
  from (select count(*),
         ...
         where wait_state not in ('waiting_for_SKGXP_receive',
                                  'waiting_for_connect',
                                  'looking_for_job')
